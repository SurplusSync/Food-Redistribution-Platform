  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-slate-500 mt-1">{t('managePickupsDeliveries')}</p>
      </div>

      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between">
          <h2 className="font-medium text-gray-900 dark:text-white">{t('assignedTasks')}</h2>
        </div>
        ...
        {!loading && donations.length === 0 && (
          <div className="p-8 text-center text-gray-500 dark:text-slate-500">{t('noAssignedTasks')}</div>
        )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500 dark:text-slate-500">{donation.quantity} {donation.unit} • {donation.donorName}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {donation.status === 'CLAIMED' && (
                      ...
                      >
                        {processingId === donation.id ? t('processing') : t('confirmPickup')}
                      </button>
                    )}
                    ...
                    {donation.status === 'PICKED_UP' && (
                      <button
                        ...
                        className="px-3 py-1 rounded-md text-xs font-medium bg-emerald-500 hover:bg-emerald-400 text-white transition-colors"
                      >
                        {processingId === donation.id ? t('processing') : t('confirmDelivery')}
                      </button>
                    )}
                  </div>
                </div>
      ...
      {selectedDonation && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-black/30 dark:bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-start gap-4 p-6">
              <div className="w-40 h-40 bg-gray-50 dark:bg-slate-950 rounded-lg overflow-hidden flex items-center justify-center relative group">
                    ...
                    {selectedDonation.imageUrls.length > 1 && (
                      <>
                        <button
                          ...
                          className="absolute left-1 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-gray-900 dark:text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                          <ChevronLeft className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => setCurrentImageIndex((prev) => (prev + 1) % selectedDonation.imageUrls.length)}
                          className="absolute right-1 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-gray-900 dark:text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                          <ChevronRight className="w-4 h-4" />
                        </button>
                        <div className="absolute bottom-2 right-2 bg-black/60 text-gray-900 dark:text-white text-xs px-2 py-0.5 rounded">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{selectedDonation.name}</h3>
                    <p className="text-gray-500 dark:text-slate-400 text-sm">{selectedDonation.quantity} {selectedDonation.unit} • {selectedDonation.foodType}</p>
                  </div>
                  <button onClick={() => { setSelectedDonation(null); setCurrentImageIndex(0); }} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-100 dark:bg-slate-800 rounded-full text-gray-500 dark:text-slate-400">
                    <X className="w-5 h-5" />
                  </button>
                </div>

                <p className="text-sm text-gray-700 dark:text-slate-300 mt-4">{selectedDonation.description || t('noAdditionalDetails')}</p>
