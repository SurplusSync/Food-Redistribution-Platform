...
export default function VolunteerDashboard() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-white">{t('volunteerDashboard')}</h1>
        <p className="text-slate-500 mt-1">{t('managePickupsDeliveries')}</p>
      </div>

      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          <h2 className="font-medium text-white">{t('assignedTasks')}</h2>
        </div>
        ...
        {!loading && donations.length === 0 && (
          <div className="p-8 text-center text-slate-500">{t('noAssignedTasks')}</div>
        )}
