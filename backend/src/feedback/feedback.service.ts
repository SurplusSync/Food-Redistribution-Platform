...
export class FeedbackService {
  ...
  async createFeedback(
    ...
    if (existingFeedback) {
      throw new BadRequestException(
        'Feedback has already been submitted for this donation',
      );
    }
    ...
    // Update the donor's trustScore
    const donorId = donation.donorId;
    const { averageScore } = await this.getAverageRatingForDonor(donorId);

    await this.userRepository.update(donorId, {
  ...
  async getAverageRatingForDonor(
    donorId: string,
  ): Promise<{ averageScore: number; totalReviews: number }> {
    ...
    const averageScore =
      totalReviews > 0
        ? parseFloat(parseFloat(result.averageScore).toFixed(1))
        : 0;
