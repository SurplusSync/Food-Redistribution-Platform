...
describe('JWT Security', () => {
    ...
    it('rejects a token signed with a different secret', () => {
        const fakeToken = jwt.sign({ sub: 'user-abc', role: 'ADMIN' }, 'wrong-secret');
    ...
    it('rejects a token whose payload was tampered with', () => {
        ...
        const tampered = Buffer.from(
            JSON.stringify({ sub: 'user-abc', role: 'ADMIN', iat: Math.floor(Date.now() / 1000) }),
        ).toString('base64url');

        expect(() => jwt.verify(`${header}.${tampered}.${signature}`, SECRET)).toThrow('invalid signature');
    });
    ...
    it('cannot escalate role from DONOR to ADMIN via token manipulation', () => {
        const donorToken = jwt.sign({ sub: 'user-abc', role: 'DONOR' }, SECRET, { expiresIn: '1h' });
    ...
    it('documents that stateless JWT cannot be invalidated before expiry without a blacklist', () => {
        const token = jwt.sign({ sub: 'user-abc', role: 'DONOR' }, SECRET, { expiresIn: '1h' });
